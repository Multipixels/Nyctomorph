...
func monster_action():
	...
	#if monster is within 3x3 of player...
	if abs(distance_from_player.x) <= 2 and abs(distance_from_player.y) <= 2:
		action_complete = true;
		var direction;
		print("monster within 3x3 of player:");
